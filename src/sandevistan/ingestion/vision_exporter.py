...
class VisionExporterError(RuntimeError):
    message: str
...
@dataclass(frozen=True)
class HTTPVisionExporterConfig:
    endpoint_url: str
    ...
    source_name: str = "http_vision_exporter"
...
@dataclass(frozen=True)
class ProcessVisionExporterConfig:
    ...
    max_clock_offset_seconds: float = 300.0
    drift_smoothing: float = 0.25
    source_name: str = "process_vision_exporter"
    source_metadata: Mapping[str, object] = field(default_factory=dict)
    default_metadata: Mapping[str, object] = field(default_factory=dict)


class HTTPVisionExporterAdapter:
    """Pull vision detections from a live camera HTTP endpoint."""

    def __init__(self, config: HTTPVisionExporterConfig, sensor_config: SensorConfig) -> None:
        self._config = config
    ...
    def fetch(self) -> List[Detection]:
        ...
        return parse_detections(normalized, self._sensor_config)
        ...
        for attempt in range(self._config.max_retries + 1):
            try:
                ...
                if attempt >= self._config.max_retries:
                    raise VisionExporterError(
                        f"Vision exporter request failed: {exc}"
                    ) from exc
                backoff = self._config.retry_backoff_seconds * (2**attempt)
                time.sleep(backoff)
        else:  # pragma: no cover - defensive
            raise VisionExporterError("Vision exporter request failed with no response.")

        try:
            payload = json.loads(body)
        except json.JSONDecodeError as exc:
            raise VisionExporterError(
                f"Vision exporter returned invalid JSON: {exc}"
            ) from exc

        if not isinstance(payload, list):
            raise VisionExporterError(
                "Vision exporter payload must be a JSON list of detections."
            )
    ...
    def _normalize_payload(
        ...
        for idx, item in enumerate(payload):
            if not isinstance(item, Mapping):
                raise VisionExporterError(
                    f"Vision exporter detection #{idx} must be an object."
                )
            entry = dict(item)
            if "camera_id" not in entry and self._config.default_camera_id:
                entry["camera_id"] = self._config.default_camera_id
    ...
    def _normalize_timestamp(self, entry: Mapping[str, object], fetch_time: float) -> float:
        ...
        if timestamp is None and "timestamp_ms" in entry:
            try:
                ...
                raise VisionExporterError(
                    "Vision exporter timestamp_ms must be numeric."
                ) from exc
        ...
        try:
            ...
            raise VisionExporterError(
                "Vision exporter timestamp must be numeric."
            ) from exc
    ...
    def _merge_metadata(self, metadata: Optional[object]) -> Mapping[str, object]:
        if metadata is None:
            ...
            raise DetectionIngestionError(
                "Vision exporter metadata must be a mapping when provided."
            )
...
class ProcessVisionExporterAdapter:
    """Pull detections from a local detector process via stdout JSON."""

    def __init__(self, config: ProcessVisionExporterConfig, sensor_config: SensorConfig) -> None:
        self._config = config
        self._sensor_config = sensor_config
        self._clock_offset_seconds = config.clock_offset_seconds
    ...
    def _fetch_payload(self) -> Iterable[Mapping[str, object]]:
        try:
            ...
            raise VisionExporterError(
                f"Vision detector command failed: {exc}"
            ) from exc
        ...
        try:
            ...
            raise VisionExporterError(
                f"Vision detector returned invalid JSON: {exc}"
            ) from exc

        if not isinstance(payload, list):
            raise VisionExporterError(
                "Vision detector payload must be a JSON list of detections."
            )
    ...
    def _normalize_payload(
        ...
        for idx, item in enumerate(payload):
            if not isinstance(item, Mapping):
                raise VisionExporterError(
                    f"Vision detector detection #{idx} must be an object."
                )
            entry = dict(item)
            if "camera_id" not in entry and self._config.default_camera_id:
                entry["camera_id"] = self._config.default_camera_id
    ...
    def _normalize_timestamp(self, entry: Mapping[str, object], fetch_time: float) -> float:
        ...
        if timestamp is None and "timestamp_ms" in entry:
            try:
                ...
                raise VisionExporterError(
                    "Vision detector timestamp_ms must be numeric."
                ) from exc
        if timestamp is None:
            timestamp = fetch_time
        try:
            ...
            raise VisionExporterError(
                "Vision detector timestamp must be numeric."
            ) from exc
    ...
    def _merge_metadata(self, metadata: Optional[object]) -> Mapping[str, object]:
        if metadata is None:
            ...
            raise DetectionIngestionError(
                "Vision detector metadata must be a mapping when provided."
            )

        merged = {"source": self._config.source_name, "command": list(self._config.command)}
        merged.update(self._config.source_metadata)
        merged.update(self._config.default_metadata)
        merged.update(base)
        return merged
